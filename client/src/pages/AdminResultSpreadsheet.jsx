...
import { 
    ...
    CheckCircle2, AlertCircle, Maximize2, Minimize2, X, Edit, FileText
} from 'lucide-react';
...
const AdminResultSpreadsheet = () => {
    ...
    const fetchAdminResults = async () => {
        try {
            const url = resultSearchQuery ? `/results?query=${resultSearchQuery}` : '/results';
    ...
    const handleManualResultSubmit = async (e) => {
        ...
        try {
            if (editingResult) {
                await api.put(`/results/${editingResult._id}`, resultData);
            ...
            setMessage({ type: 'error', text: "Failed to save result. " + (error.response?.data?.message || error.message) });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className={`flex flex-col bg-gray-100 ${fullScreen ? 'fixed inset-0 z-[100]' : 'min-h-screen pt-20'}`}>
            ...
            <div className="flex-1 overflow-auto p-6">
                ...
                {/* Student Results Database Section */}
